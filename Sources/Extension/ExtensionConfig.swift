...
struct MeditationExtensionConfig: AppExtensionConfiguration {
    func accept(connection: NSXPCConnection) -> Bool {
        ...
        // Surfaces belong to wallpaper IDs and survive routine XPC reconnects.
        connection.invalidationHandler = { extensionLog("XPC connection closed; surfaces retained") }
        handler.agentProxy = connection.remoteObjectProxy as? WallpaperExtensionProxyXPCProtocol
        connection.resume()
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) { [weak handler] in
            guard let models = buildSettingsViewModelsXPC() else { return }
            handler?.agentProxy?.updateSettingsViewModels(models) { error in
                extensionLog(error.map { "Picker update failed: \($0.localizedDescription)" } ?? "Published all shader choices to WallpaperAgent")
            }
        }
        return true
    }
